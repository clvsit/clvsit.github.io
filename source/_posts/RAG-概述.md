...
TREC（Text REtrieval Conference）是一个由美国国家标准与技术研究院（NIST）组织的一系列信息检索领域的研究会议和评估活动。自1992年以来，TREC每年举办一次，旨在推动信息检索技术的发展，包括但不限于搜索引擎、数据挖掘、自然语言处理等技术。

TREC会议提供了一个标准化的测试平台，研究人员可以在这个平台上测试和比较他们的算法和系统。参与者通常需要使用TREC提供的大规模文本数据集来评估他们的检索系统的性能。TREC的评估活动包括各种任务，如网页搜索、问答系统、跨语言检索等。
